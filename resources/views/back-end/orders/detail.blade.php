        ...
        <div class="row">
            <div class="col-lg-12">
                <form action="" method="POST" role="form">
                                ...
                                <table class="table table-hover">
                                    ...
                                    @foreach($data as $row)
                                        <tr>
                                            ...
                                            <td>
                                                <input type="hidden" name="sub_price{{$row->id}}"
                                                       value="{{ $row->price *  $row->qty}}">
                                                <a href="{!!route('getOrderList') .'/remove/'. $row->id!!}" title="Xóa"
                    ...
                    @if($oder->status)
                        <a href="{{route('getOrderList')}}" class="btn btn-info">Trở Về</a>
                    @else
                        <button type="submit" onclick="return xacnhan('Xác nhận đơn hàng này ?')"
                                class="btn btn-danger"> Xác nhận đơn hàng
                        </button>
                    @endif
                </form>
            </div>
